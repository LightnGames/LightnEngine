...
void DebugGeomtryRenderer::initialize(ComPtr<ID3D11Device>& device)
{
	...
	for (int v = 0; v <= v_max; v++) {
		for (int u = 0; u < u_max; u++) {
			double theta = radianFromDegree(180.0f * v / v_max);
			double phi = radianFromDegree(360.0f * u / u_max);
			double x = sin(theta) * cos(phi);
			double y = cos(theta);
			double z = sin(theta) * sin(phi);
			vSphere[u_max * v + u] = Vector3(x, y, z)/2;
		}
	}
	...
	_sphereMesh->initialize(vSphere.data(), vSphere.size() * sizeof(Vector3), iSphere.data(), iSphere.size(), device);
...
void DebugGeomtryRenderer::draw(const DebugDrawList & sphere, const DebugDrawList & box, const DebugDrawList & line, const DrawSettings & drawSettings)
{
	...
	if (!sphere.empty()) {
		_sphereMesh->draw(drawSettings, sphere.data(), sphere.size());
	}
	...
	if (!box.empty()) {
		_boxMesh->draw(drawSettings, box.data(), box.size());
	}
	...
	if (!line.empty()) {
		_lineMesh->draw(drawSettings, line.data(), line.size());
	}
