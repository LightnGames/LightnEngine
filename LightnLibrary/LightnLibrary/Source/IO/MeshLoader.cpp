...
HRESULT MeshLoader::load(const std::string & filePath, const std::vector<std::string>& matFiles) {
	...
	//メッシュパスを取得
	const auto path_slash = filePath.find_last_of("/") + 1;
	_folderPath = "Resources/" + filePath.substr(0, path_slash);
	_filePath = filePath;

	const auto path_point = filePath.find_last_of(".") + 1;
...
HRESULT MeshLoader::createShaderAndConstantBuffer(std::shared_ptr<MaterialData>& material, bool isSkeletalMesh) {
	...
	//頂点シェーダーを読み込み
	RendererUtil::createVertexShader(material->vertexShaderFileName.c_str(), material->pVertexShader, layout.data(), static_cast<uint32>(layout.size()), material->pVertexLayout, _device);
