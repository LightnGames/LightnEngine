...
void TileBasedLightCulling::draw(const DrawSettings& settings,
	...
	const uint16 width = static_cast<uint16>(deferredBuffers->getGBufferSize().x);
	const uint16 height = static_cast<uint16>(deferredBuffers->getGBufferSize().y);
	const uint16 dispatchWidth = static_cast<uint16>((width + COMPUTE_SHADER_TILE_GROUP_DIM - 1) / COMPUTE_SHADER_TILE_GROUP_DIM);
	const uint16 dispatchHeight = static_cast<uint16>((height + COMPUTE_SHADER_TILE_GROUP_DIM - 1) / COMPUTE_SHADER_TILE_GROUP_DIM);
